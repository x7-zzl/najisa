...
**Najisa**（天地一家大爱盟）是一个以国产网络小说 **《蛊真人》**（Reverend Insanity）为世界观背景的**动漫论坛社区**。用户可以在平台上浏览、讨论虚拟的"蛊虫"生物，发表文章与视频，使用 AI 助手，并体验一个完整的社区互动生态。
...
| 板块 | 路由 | 功能描述 |
|------|------|----------|
| **蛊界佚闻** | `/system/gujie` | 论坛文章列表，支持发布、搜索、置顶、热门标记（日浏览量 ≥ 100）、评论系统 |
| **众生万象** | `/system/zhongsheng` | 视频作品展示，封面预览，点赞/收藏/投币互动 |
| **宝黄天** | `/system/baohuang` | 图片画廊模块 |
| **天下名蛊** | `/system/tianxia` | **核心功能**：蛊虫百科图鉴，按类别（凡蛊/仙蛊）、流派（37 种）、转数（1-9 转）多维度筛选，卡片式网格展示 + 图片轮播 |
